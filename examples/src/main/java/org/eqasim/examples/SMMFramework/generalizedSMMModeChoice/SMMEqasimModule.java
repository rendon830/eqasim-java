 ...
 */
public class SMMEqasimModule extends AbstractEqasimExtension {
	 ...
	 */
	public SMMEqasimModule(CommandLine commandLine, Scenario scenario, HashMap<String, String> service) {
	...
	public void bindBikeShareService(String name, String mode) {
		...
		try {
			bindCostModel("sharing:" + name).toInstance(new org.eqasim.examples.SMMFramework.generalizedSMMModeChoice.generalizedSMMModeChoice.costModels.SMMBikeShareCostModel("sharing:" + name, this.provideCostParameters((EqasimConfigGroup) this.scenario.getConfig().getModules().get("eqasim"), this.commandLine)));
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		}
   ...
   public SMMEScooterCostModel provideEScooterCostModel(EqasimConfigGroup config, String name){
	 ...
	 */
	public SMMEScooterEstimator provideEScooterEstimator(String name, SMMParameters modeParameters, SMMEScooterPredictor predictor, SMMPersonPredictor personPredictor){
